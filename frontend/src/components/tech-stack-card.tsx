...
// Displays Phase 6 stack detection with confidence and evidence. Logos are kept
// as compact text badges for now so the UI stays dependency-free and consistent.
...
// Renders one technology row with a compact text logo, confidence, category,
// and evidence bubbles from the backend detector.
    ...
    <li className="rounded-md border border-slate-200 bg-slate-50 p-4">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        ...
        <span className="inline-flex w-fit shrink-0 items-center rounded-md border border-emerald-200 bg-emerald-50 px-3 py-1 font-mono text-sm font-semibold text-emerald-800">
          {percentFormatter.format(technology.confidence)}
        </span>
      </div>
      ...
      <ul className="mt-4 flex flex-wrap gap-2">
        {technology.evidence.map((evidence) => (
          <li
            className="max-w-full rounded-md border border-slate-200 bg-white px-3 py-2 text-sm leading-6 text-slate-600"
            key={`${evidence.source}-${evidence.detail}-${evidence.path ?? ""}`}
          >
