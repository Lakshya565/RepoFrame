...
// boundary, so this module validates both the bundled month/year contract and
// the former single-range contract that can appear during a staggered deploy.
...
// a render-time property access and crashing the entire Analysis route. A valid
// single-range response remains distinguishable so the API helper can request and
// combine only the missing range.
...
// Combines two independently validated legacy responses. Requiring matching
// repository identity and different ranges prevents deployment compatibility
// from weakening the current response contract.
